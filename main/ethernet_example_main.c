...
#ifdef CONFIG_EMBED_CLIENT_CERT
                                 ...
                                 int32_t event_id, void *event_data)
{
    ...
    // Start OTA update check after getting IP
    // You can trigger OTA updates here or set up a timer
    ESP_LOGI(TAG, "Ethernet connected, ready for OTA updates");
    
    // For testing: Trigger OTA update after 10 seconds
    // Comment this out in production
    ESP_LOGI(TAG, "Scheduling OTA update check in 10 seconds...");
    vTaskDelay(pdMS_TO_TICKS(10000));
    ESP_LOGI(TAG, "Starting OTA update check...");
    esp_err_t ret = ota_manager_start_update();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to start OTA update: %s", esp_err_to_name(ret));
    }
}

// OTA event callback function
static void ota_event_handler(ota_event_data_t *event_data)
{
    switch (event_data->event) {
        ...
        case OTA_EVENT_PROGRESS:
            ESP_LOGI(TAG, "OTA progress: %d%%", event_data->progress_percent);
            break;
        case OTA_EVENT_COMPLETED:
            ESP_LOGI(TAG, "OTA update completed successfully");
            break;
        case OTA_EVENT_FAILED:
            ESP_LOGE(TAG, "OTA update failed: %s", event_data->error_message);
            break;
    }
}
...
void app_main(void)
{
    ...
    ota_config_t ota_config = {
        ...
        .cert_pem = NULL,              // Will be set if certificate exists
        .client_cert_pem = NULL,       // Will be set if client cert exists
        .client_key_pem = NULL,        // Will be set if client key exists
        .timeout_ms = CONFIG_OTA_UPDATE_TIMEOUT_MS,
        .skip_cert_common_name_check = false,  // Enable certificate validation for security
    };
    
    // Set certificates if they exist (optional - certificate bundle will be used if not provided)
#ifdef CONFIG_EMBED_SERVER_CERT
    ota_config.cert_pem = server_cert_pem_start;
#endif

#ifdef CONFIG_EMBED_CLIENT_CERT
    ota_config.client_cert_pem = client_cert_pem_start;
    ota_config.client_key_pem = client_key_pem_start;
#endif
